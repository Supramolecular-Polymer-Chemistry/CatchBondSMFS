function [edges,values,frac,time] = expftthresh(F_ev,tau_ev,binsize,threshold)
...
for k = 1:length(values)
    ...
    if ~isempty(lifetimes(lifetimes>threshold))
        lifetimes = sort(lifetimes(lifetimes>threshold));
